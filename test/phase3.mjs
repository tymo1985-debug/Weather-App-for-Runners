...
const cache = name => {
  ...
  return {
    addAll: async paths => paths.forEach(path => entries.set(key(path), new Response('shell'))),
...
assert.equal(await (await request(shell + 'route', 'navigate')).text(), 'shell');
